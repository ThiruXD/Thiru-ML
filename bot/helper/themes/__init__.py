...
from bot.helper.themes import thiruml_minimal

AVL_THEMES = {}
for theme in listdir('bot/helper/themes'):
    if theme.startswith('thiruml_') and theme.endswith('.py'):
...
def BotTheme(var_name, **format_vars):
    ...
    if theme_ in AVL_THEMES:
        text = getattr(AVL_THEMES[theme_].THIRUMLStyle(), var_name, None)
        if text is None:
        ...
        text = getattr(rantheme.THIRUMLStyle(), var_name, None)
        
    if text is None:
        text = getattr(thiruml_minimal.THIRUMLStyle(), var_name)
